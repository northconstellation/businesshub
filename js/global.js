...
if($.cookie("hub4")){
	var dateEnd = $.cookie("hub4");
}else{
	var dateEnd = returnEndDate(0,24,0);
	$.cookie("hub4", dateEnd, {expires: 3});
};
...
$('.navigation__bar__btn').click(function(){
	...
	// load more comments on page load
	(function(){
		$.get('more-testimonials.html', function(data) {
			if (data) $('.testimonials_list').append(data);	
		});
	}());

    // testimonials slider
    $('.owl-slider-testimonails').owlCarousel({
        items:1,
        margin:0,
        nav: true,
        navContainer: '.testimonils-slider--nav',
    });
